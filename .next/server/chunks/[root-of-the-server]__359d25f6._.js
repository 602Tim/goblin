...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/prisma.ts [app-route] (ecmascript)"); // 確保這是正確的路徑
// 導入 PrismaAdapter
...
;
;
// ✨ 新增：檢查 prisma 實例是否正確載入
console.log("Auth Config - Prisma client upon import:", __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"]);
if (!__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"]) {
    console.error("Auth Config - ERROR: Prisma client is undefined upon import. Database operations will fail.");
}
;
const commonOAuthParams = {
    prompt: "consent",
    access_type: "offline",
    response_type: "code"
};
const SPECIAL_ROLES_MAPPING = {
    'a0903992205@gmail.com': 'CHEF',
    'robbie992667@gmail.com': 'OWNER',
    'hibiki992667@gmail.com': 'STAFF',
    'pelito4427@gmail.com': 'CUSTOMER'
};
const authOptions = {
    // 確保這裡有 adapter 配置，並且 prisma 實例是有效的
    ...
    callbacks: {
        async jwt ({ token, user, account, profile }) {
            console.log("--- JWT Callback Triggered (v4) ---");
            console.log("JWT - Current Token state upon entry:", JSON.parse(JSON.stringify(token)));
            console.log("JWT - Incoming User object from Provider:", user); // ✨ 這時 user 應該會有值了
            console.log("JWT - Incoming Account object:", account);
            console.log("JWT - Incoming Profile object (OAuth):", profile);
            const email = user?.email || profile?.email || token.email;
            console.log("JWT - Extracted Email for lookup:", email);
            if (email) {
                // 如果 adapter 配置正確，user 應該就是從資料庫來的，可以直接使用其 role
                // 但為了確保邏輯健壯性，仍然可以從資料庫再次確認或覆寫角色
                // 注意：這裡的 prisma.user.findUnique 是你自己定義的，
                // 如果 Adapter 已經正確工作，它也會將用戶寫入 DB。
                let dbUser = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].user.findUnique({
                    where: {
                        email: email
                    }
                });
                console.log("JWT - DB User Found (based on email):", dbUser);
                if (dbUser) {
                    console.log("JWT - dbUser.id from DB:", dbUser.id);
                    console.log("JWT - dbUser.role from DB:", dbUser.role);
                    token.id = dbUser.id;
                    token.role = dbUser.role; // 從資料庫設置角色
                    token.provider = account?.provider;
                    console.log("JWT - Token updated with DB User ID and Role:", JSON.parse(JSON.stringify(token)));
                } else {
                    // 如果資料庫中沒有這個用戶 (可能是首次登入，Adapter 會創建)
                    const mappedRole = SPECIAL_ROLES_MAPPING[email];
                    token.role = mappedRole || 'CUSTOMER'; // 設為預設顧客
                    console.warn(`JWT - User with email "${email}" not found in DB. Assigned mapped role "${mappedRole || 'undefined'}" or default role: ${token.role}.`);
                    console.log("JWT - Token after default CUSTOMER assignment:", JSON.parse(JSON.stringify(token)));
                }
            } else {
                console.warn("JWT - Email not found in user, profile, or initial token object. Role cannot be assigned by DB lookup.");
            }
            console.log("--- JWT Callback Finished, Returning Token:", JSON.parse(JSON.stringify(token)));
            return token;
        },
        async session ({ session, token }) {
            console.log("--- Session Callback Triggered (v4) ---");
            console.log("Session - Incoming Token:", JSON.parse(JSON.stringify(token)));
            if (token && session?.user) {
                const userWithCustomProps = session.user;
                userWithCustomProps.id = token.id;
                userWithCustomProps.role = token.role;
                userWithCustomProps.provider = token.provider;
                console.log("Session - Session User updated:", JSON.parse(JSON.stringify(session.user)));
            } else {
                console.warn("Session - Token or session.user missing. Session not fully populated.");
            }
            console.log("--- Session Callback Finished (v4) ---");
            return session;
        }
    },
    pages: {
...
// ✨ 修改這裡：從 "next-auth/next" 導入 NextAuth
__turbopack_context__.s({
    "GET": (()=>handler),
    "POST": (()=>handler)
});
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2d$auth$2f$next$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next-auth/next/index.js [app-route] (ecmascript)"); // ✨ 修改這行！
var __TURBOPACK__imported__module__$5b$project$5d2f$auth$2e$config$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/auth.config.ts [app-route] (ecmascript)"); // 導入你的認證配置 (auth.config.ts 應該預設導出 authOptions)
;
;
// NextAuth 函數接收你的 authOptions 配置，並返回一個處理器
const handler = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2d$auth$2f$next$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"])(__TURBOPACK__imported__module__$5b$project$5d2f$auth$2e$config$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"]);
;
 // 如果你的專案未來可能需要其他 HTTP 方法 (如 PUT, DELETE 等)，
 // 也可以這樣導出：
...
//# sourceMappingURL=%5Broot-of-the-server%5D__359d25f6._.js.map