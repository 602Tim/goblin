...
// app/api/order/[orderId]/status/route.js
__turbopack_context__.s({
    "PATCH": (()=>PATCH)
});
...
async function PATCH(req, { params }) {
    ...
    const { orderId: orderId1 } = params;
    const { status: status1 } = await req.json();
    if (!orderId1 || !status1) {
        return new Response("缺少必要資料", {
            status: 400
        });
    }
    try {
...
try {
    ...
    if (status === "READY" || status === "COMPLETED") {
        ...
        mqtt.publish(topic, JSON.stringify({
            ...
            if (err) {
                console.error("❌ 發送狀態通知失敗:", err);
            } else {
                console.log(`📢 已通知顧客訂單狀態: ${status}`);
            }
        });
    }
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(updated);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__94e4d0cd._.js.map